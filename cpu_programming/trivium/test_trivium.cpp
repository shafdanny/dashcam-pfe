...
unsigned char get_byte(int** r1,int** r2,int** r3)
{
	int i;
	int t1,t2,t3;
	int bit1,bit2,bit3;
...
*/
char* generate_random_key(int len,int* key,int* init)
{
	...
	for(i=0;i<len;i++){
		res[i] = get_byte(&regist1,&regist2,&regist3);
	}
	return res;
}

int* generate_random_ikey(int len,int* key,int* init)
{
	std::cout << "len : " << len << std::endl;
	int i;
...
{
    int i;
	int* key = (int*)calloc(3,sizeof(int));
    ...
    char* res = generate_random_key(len,key,init);
	int* tmp = generate_random_ikey(len/4,key,init);
    ...
    keyFile2.write(reinterpret_cast<const char*>(tmp), len);
