...
//g++ -o decrypt decrypt.cpp trivium.cpp


int main(){
    
    FILE *fp;
    fp = fopen("image_crypted_test1.jpg","rb");  // r for read, b for binary
    fseek(fp, 0, SEEK_END);
    ...
    fp = fopen("image_decrypted_test1.jpg", "wb");
    fwrite(result, sizeof(result[0]), filelen/sizeof(result[0]), fp);
    fclose(fp);
    
    
    fp = fopen("image_crypted_test2.jpg","rb");  // r for read, b for binary
    fseek(fp, 0, SEEK_END);
    filelen = ftell(fp);
    rewind(fp);
    ...
    fp = fopen("image_decrypted_test2.jpg", "wb");
    fwrite(result, sizeof(result[0]), filelen/sizeof(result[0]), fp);
    fclose(fp);
