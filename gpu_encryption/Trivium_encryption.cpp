...
{
	int i;
	unsigned int* res =(unsigned int*)malloc(n*sizeof(int));
	for(i=0;i<len;i++){
		res[i] = message[i] ^ get_integer(r1,r2,r3);
	}
	return res;
}
...
int main()
{
  ...
  //Data data = read("image_crypted_test.jpg");
  Data data = read("images/ornito.jpg");
	int len = data.length;
	int nb_integer = LEN(data.length);
	int nb_vector = LENVECTOR(data.length);
  
  printf("size of file %d byte integer %d vector %d\n",data.length,nb_integer,nb_vector);
  ...
  int* regist3 = (int*)calloc(4,sizeof(int));
	...
	k.setNumQPUs(1);
	SharedArray<int> message(5), key(160), encrypted(5);
	char* test = "salut ca va connard";
	//int* integer_msg = convert_to_integer(data.buffer,len);
	int* integer_msg = convert_to_integer(test,20);
	unsigned int* cpu_encrypted = trivium_cpu(integer_msg,&regist1,&regist2,&regist3,5);
	for(i=0;i<5;i++){
		printf("message cpu %x\n",cpu_encrypted[i]);
	}
	/*
	for(i=0;i<20;i++){
	...
	k(&message, &key, &encrypted,1);
	for(i=0;i<16;i++){
		printf("result cpu %x\n",get_integer(&regist1,&regist2,&regist3));
	}	
  ...
  fp = fopen("image_crypted_test.jpg", "wb");
